...
import { useShuttleAnalytics, useShuttleLocations } from '@/hooks/useDashboard';
    ...
    <div className="p-6 rounded-2xl bg-[#18181B] border border-[#27272A] space-y-6 shadow-xl">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-base font-bold text-white tracking-tight">Shuttle Fleet Tracking</h3>
            <p className="text-xs text-[#A1A1AA]">Live campus transit telemetry & stop ETAs</p>
          </div>
        </div>

        <div className="text-right">
          <span className="text-2xl font-black text-emerald-400">{analytics?.activeShuttles ?? 0}</span>
          <span className="block text-[10px] text-[#A1A1AA]">Active Shuttles</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        <div className="p-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
          ...
          <span className="text-[10px] text-[#A1A1AA]">Operating</span>
        </div>
        <div className="p-2.5 rounded-xl bg-[#09090B] border border-[#27272A]">
          <span className="text-xs font-bold text-white block">{analytics?.inactiveShuttles ?? 0}</span>
          <span className="text-[10px] text-[#A1A1AA]">Inactive</span>
        </div>
              ...
              >
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-white">{shuttle.shuttleName || shuttle.shuttleNumber}</span>
                ...
                <div className="text-right">
                  ...
                  <span className="text-[10px] text-[#71717A] block">{shuttle.direction || 'In Transit'}</span>
                </div>
              </div>
            ))}
          </div>
