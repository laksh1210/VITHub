...
import { useCanteenQueues } from "@/hooks/api/use-canteen-queues";
...
function QueuesPageContent() {
  ...
  return (
    <div className="space-y-6">
      ...
      {!isLoading && !isError && filtered.length === 0 && (
        <EmptyState
          icon={queues && queues.length > 0 ? <SearchX className="h-10 w-10 text-muted-foreground" /> : <TimerReset className="h-10 w-10 text-muted-foreground" />}
