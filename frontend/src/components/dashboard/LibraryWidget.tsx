    ...
    <div className="p-6 rounded-2xl bg-card border border-border space-y-6 shadow-xl">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-base font-bold text-foreground tracking-tight">Library Seat Availability</h3>
            <p className="text-xs text-muted-foreground">Real-time study seat capacity & reservation status</p>
          </div>
        </div>
        ...
        {seatTypes.map((seat) => {
          ...
          return (
            <div key={seat.label} className={`p-3 rounded-xl border ${seat.bg} flex items-center gap-2.5`}>
              ...
              <div>
                <span className="text-xs font-bold text-foreground block">{seat.count}</span>
                <span className="text-[10px] text-muted-foreground">{seat.label}</span>
              </div>
            </div>
          );
        })}
      ...
      {/* Footnote */}
      <div className="p-3 rounded-xl bg-background border border-border flex items-center justify-between text-xs text-muted-foreground">
        <span>Monitored Libraries</span>
        <span className="font-semibold text-foreground">{analytics.totalLibraries} Campus Central Libraries</span>
      </div>
    </div>
  );
}
