...
import { useNotifications } from '@/hooks/api/use-dashboard';
    ...
    <div className="p-6 rounded-2xl bg-card border border-border space-y-6 shadow-xl">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-base font-bold text-foreground tracking-tight">System Notifications</h3>
            <p className="text-xs text-muted-foreground">Real-time operational alerts & campus notices</p>
          </div>
        </div>

        <div className="text-right">
          <span className="text-2xl font-black text-indigo-400">{unreadCount}</span>
          <span className="block text-[10px] text-muted-foreground">Unread Notices</span>
        </div>
      </div>
