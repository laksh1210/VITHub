...
import { useOccupancyAnalytics, useRoomOccupancies } from '@/hooks/api/use-dashboard';
    ...
    <div className="p-6 rounded-2xl bg-card border border-border space-y-6 shadow-xl">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-base font-bold text-foreground tracking-tight">Classroom Occupancy</h3>
            <p className="text-xs text-muted-foreground">Real-time room availability & density metrics</p>
          </div>
        </div>

        {/* Global Occupancy Percentage Badge */}
        <div className="text-right">
          <span className="text-2xl font-black text-foreground">{occupancyRate.toFixed(1)}%</span>
          <span className="block text-[10px] text-muted-foreground">Campus Avg Density</span>
        </div>
      </div>
        ...
        {statusBadges.map((status) => {
          ...
          return (
            <div key={status.label} className={`p-3 rounded-xl border ${status.bg} flex items-center gap-2.5`}>
              ...
              <div>
                <span className="text-xs font-bold text-foreground block">{status.count}</span>
        ...
        <div className="space-y-2 pt-2 border-t border-border">
          ...
          <div className="space-y-2 max-h-44 overflow-y-auto pr-1">
            {rooms.slice(0, 4).map((room) => (
              ...
              >
                <div>
                  <span className="font-semibold text-foreground">{room.roomName || room.roomNumber}</span>
                  <span className="text-[10px] text-muted-foreground ml-2">({room.buildingName})</span>
                </div>
                <div className="flex items-center gap-3">
