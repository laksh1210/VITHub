...
import { useCanteenAnalytics, useCanteenQueues } from '@/hooks/api/use-dashboard';
    ...
    <div className="p-6 rounded-2xl bg-card border border-border space-y-6 shadow-xl">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="p-3 rounded-xl bg-background border border-border flex items-center gap-3">
          ...
          <div>
            <span className="text-xs font-extrabold text-foreground block">{avgQueue.toFixed(1)} Persons</span>
            <span className="text-[10px] text-muted-foreground">Avg Queue Length</span>
          </div>
        </div>
        <div className="p-3 rounded-xl bg-background border border-border flex items-center gap-3">
          <Clock className="w-4 h-4 text-emerald-400" />
          <div>
            <span className="text-xs font-extrabold text-foreground block">{analytics?.activeCanteens ?? 0} Active</span>
            <span className="text-[10px] text-muted-foreground">Open Canteens</span>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-2 pt-2 border-t border-border">
          ...
          <div className="space-y-2 max-h-44 overflow-y-auto pr-1">
            {queues.slice(0, 4).map((canteen) => (
              ...
              >
                <div>
                  <span className="font-semibold text-foreground">{canteen.canteenName}</span>
                  <span className="text-[10px] text-muted-foreground ml-2">({canteen.buildingName})</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className="text-right">
                    ...
                    <span className="text-[10px] text-muted-foreground">~{canteen.estimatedWaitMinutes} mins</span>
                  </div>
                  <span
